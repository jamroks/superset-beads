...
mkdir -p packages/task-provider
...
# ─── Merge apps/desktop/package.json ───────────────────────────────────────
section "Merging apps/desktop/package.json"
...
if (!pkg.dependencies['@superset/task-provider']) {
  pkg.dependencies['@superset/task-provider'] = 'workspace:*'
  console.log('  + dependencies.@superset/task-provider')
} else {
  console.log('  ~ dependencies.@superset/task-provider already set — skipping')
}
...
writeFileSync(pkgPath, JSON.stringify(pkg, null, '\t') + '\n')
console.log('  apps/desktop/package.json merged successfully')
EOF

success "apps/desktop/package.json merged"
...
bun run typecheck && success "TypeScript OK" || warn "TypeScript errors — inspect the fork additions and current upstream drift"
